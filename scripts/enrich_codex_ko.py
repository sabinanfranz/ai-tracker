...
RECORDS: list[tuple[str, str, str]] = [
    ...
    (
        ...
        "원격 연결이 더 안정적이고, Windows 터미널 단축키가 수정됐어요",
        "원격 연결 시 오류 메시지가 더 명확해지고, 상태 업데이트와 호스트 라벨이 개선되었습니다.\n"
        "Windows 통합 터미널에서 복사/붙여넣기 단축키가 수정되었습니다.\n"
        "보관된 고정 스레드를 고정 해제할 수 없던 문제가 해결되었습니다.",
    ),
    (
        "2ede8f93-b5e2-434e-8cde-44e5e728b03c",
        "채팅에서 @플러그인으로 바로 참조하고, 모델 선택이 편리해졌어요",
        "@plugin 멘션으로 채팅에서 플러그인을 직접 참조하고 관련 MCP/앱/스킬 컨텍스트를 자동 포함할 수 있습니다.\n"
        "TUI 모델 선택기에 최신 모델 카탈로그 변경사항이 바로 반영됩니다.\n"
        "실행 환경이 통합되고 멀티 에이전트 안정성이 개선되었습니다.",
    ),
    (
        "8f0cfc99-ea4d-4034-a747-642a58bad6d7",
        "실행 중에 추가 권한을 요청할 수 있고, 플러그인 마켓이 생겼어요",
        "실행 중인 턴에서 추가 권한을 요청할 수 있는 내장 도구가 추가되고, 승인 UI가 새로 만들어졌습니다.\n"
        "큐레이션된 마켓플레이스 탐색, 풍부한 메타데이터, 설치 시 인증이 추가되었습니다.\n"
        "사전에 전체 접근 권한을 부여하지 않아도 안전한 에이전트 실행이 가능해졌습니다.",
    ),
    (
        "e3e58cdc-0a76-4013-8310-0861e1a8e034",
        "실험적 코드 모드와 훅 엔진이 추가되고, 헬스체크가 쉬워졌어요",
        "더 격리된 코딩 워크플로를 위한 실험적 코드 모드가 추가되었습니다.\n"
        "SessionStart/Stop 이벤트를 지원하는 실험적 훅 엔진이 도입되었습니다.\n"
        "WebSocket 서버 배포 시 동일 포트에서 /readyz, /healthz 엔드포인트를 사용할 수 있습니다.",
    ),
    (
        "adab76d5-2e8a-4074-9510-ba48f029214e",
        "Codex가 터미널 출력을 읽어서 상황을 파악할 수 있게 됐어요",
        "현재 스레드의 통합 터미널을 읽어 실행 중인 개발 서버 상태나 빌드 오류를 확인할 수 있습니다.\n"
        "에이전트가 실시간 터미널 컨텍스트를 활용해 사용자와 함께 작업할 수 있습니다.\n"
        "추가적인 성능 개선과 버그 수정이 포함되어 있습니다.",
    ),
    (
        "3bcc9c9f-19e4-4590-ba1d-4fac811876c5",
        "나만의 테마를 꾸미고, 자동화를 로컬/원격으로 선택할 수 있어요",
        "테마 색상, 배경, 글꼴을 직접 설정하고 친구와 공유할 수 있는 커스텀 테마가 추가되었습니다.\n"
        "자동화 실행 시 로컬과 원격 환경을 선택할 수 있도록 개편되었습니다.\n"
        "완전히 개인화된 워크스페이스와 유연한 자동화 설정이 가능해졌습니다.",
    ),
    (
        "ea61ff3e-0431-493a-85a9-f03231c59212",
        "고해상도 이미지 분석과 js_repl 세션 유지 기능이 추가됐어요",
        "모델이 view_image와 codex.emitImage를 통해 고해상도 이미지를 요청하여 정밀한 시각 작업이 가능합니다.\n"
        "js_repl에서 codex.cwd와 codex.homeDir이 노출되고, 저장된 도구 호출이 세션 간 유지됩니다.\n"
        "멀티모달 시각 디버깅과 영구적인 REPL 상태 유지 기능이 강화되었습니다.",
    ),
    (
        "7a945a39-8f4e-4bae-aec2-3492c367cabb",
        "GPT-5.4 mini 모델로 가벼운 코딩 작업을 더 빠르게 할 수 있어요",
        "GPT-5.4 mini가 Codex에 추가되어 가벼운 코딩 작업과 서브에이전트에 활용할 수 있습니다.\n"
        "GPT-5 mini 대비 코딩, 추론, 이미지 이해, 도구 사용 성능이 향상되고 2배 이상 빨라졌습니다.\n"
        "GPT-5.4 대비 30%의 한도만 사용하여 동일 작업을 약 3.3배 더 오래 수행할 수 있습니다.",
    ),
    (
        "f60b6515-f155-4acf-aa83-e9baa07b7224",
        "ChatGPT 로그인과 플러그인 설정이 더 편리해졌어요",
        "온보딩 중 ChatGPT 디바이스 코드 로그인을 지원하고 기존 토큰을 갱신할 수 있습니다.\n"
        "누락된 플러그인 설치를 안내하고 허용 목록 설정 및 원격 동기화가 가능합니다.\n"
        "user-prompt-submit 훅이 추가되어 프롬프트 실행 전 차단하거나 수정할 수 있습니다.",
    ),
    (
        "6d560a8b-79e5-4e3a-b344-48a5f039e93e",
        "뒤로/앞으로 버튼으로 빠르게 이동해요",
        "헤더에 뒤로·앞으로 버튼이 추가되어 최근 화면 간 이동이 빨라졌습니다.\n"
        "스레드 메뉴에서 Finder/탐색기/파일 관리자로 바로 열 수 있습니다.\n"
        "스레드 오류 메시지가 더 명확해져서 문제 파악이 쉬워졌습니다.",
    ),
    (
        "a1021ca2-8647-4a3c-9409-192b2b23d0f1",
        "이전 메시지에서 대화를 분기할 수 있어요",
        "최신 턴뿐 아니라 이전 메시지에서도 대화를 분기할 수 있습니다.\n"
        "슬래시 명령으로 모델과 추론 수준을 전환할 수 있습니다.\n"
        "플랜 모드에서 입력이 필요하면 알림이 표시됩니다.",
    ),
    (
        "9b679ced-3744-400d-989e-59c74252639f",
        "@ 메뉴에서 스킬을 바로 삽입할 수 있어요",
        "@ 메뉴에 스킬이 추가되어 작성 창에서 바로 삽입할 수 있습니다.\n"
        "Cmd/Ctrl+F 검색 시 현재 선택한 텍스트가 자동으로 입력됩니다.\n"
        "추가적인 성능 개선과 버그 수정이 포함되었습니다.",
    ),
    (
        "28c3194f-f50c-4b97-92a4-bd351eb7f70d",
        "스레드 검색과 일괄 보관 기능이 추가됐어요",
        "과거 스레드를 검색하고 키보드 단축키로 빠르게 이동할 수 있습니다.\n"
        "프로젝트 내 모든 로컬 스레드를 한 번에 보관할 수 있습니다.\n"
        "Codex 앱과 VS Code 확장 간 주요 설정이 동기화됩니다.",
    ),
    (
        "0439ab54-453b-41fe-b97d-1f18b01b7c0c",
        "Codex에서 플러그인을 설치하고 관리할 수 있어요",
        "스킬, 앱 연동, MCP 서버 설정을 묶은 플러그인을 설치할 수 있습니다.\n"
        "Codex 앱, CLI, IDE 확장에서 큐레이션된 플러그인 디렉터리를 이용합니다.\n"
        "@plugin-creator로 로컬 플러그인을 직접 만들고 테스트할 수 있습니다.",
    ),
    (
        "f6663849-8a9e-4a70-b660-974f82c4fa63",
        "플러그인이 정식 기능으로 업그레이드됐어요",
        "시작 시 플러그인 동기화, /plugins 탐색, 인증·설정이 개선됐습니다.\n"
        "서브 에이전트가 /root/agent_a 같은 경로 주소를 사용합니다.\n"
        "플러그인 설치·제거·원격 동기화 등 생애주기 관리가 향상됐습니다.",
    ),
    # ── batch added 2026-04-26 ──
    (
        "cf8c51a4-2b01-4b4d-ac59-57ed7e3890f2",
        "Windows 프록시 샌드박스와 동적 인증 토큰이 추가됐어요",
        "Windows 샌드박스가 OS 수준 송신 규칙으로 프록시 전용 네트워킹을 강제합니다.\n"
        "앱 서버 클라이언트가 디바이스 코드로 ChatGPT 로그인을 시작하고 codex exec가 프롬프트와 stdin을 함께 받습니다.\n"
        "사용자 지정 모델 제공자가 짧은 베어러 토큰을 동적으로 갱신할 수 있습니다.",
    ),
    (
        "ee46ada6-cec1-4271-b72f-903cfbed1943",
        "ChatGPT 로그인에서 구형 GPT-5.x 모델이 제거돼요",
        "4월 7일부터 모델 선택기에서 gpt-5.2-codex, gpt-5.1 계열, gpt-5가 사라집니다.\n"
        "4월 14일에 ChatGPT 로그인 사용자는 해당 모델을 더 이상 사용할 수 없습니다.\n"
        "gpt-5.4, gpt-5.4-mini, gpt-5.3-codex, gpt-5.2는 계속 이용할 수 있습니다.",
    ),
    (
        "b009f808-ee86-46b4-9da2-fd56d6d0bd03",
        "실시간 음성이 v2 WebRTC로 기본 전환되고 exec-server가 추가됐어요",
        "실시간 음성 세션이 v2 WebRTC 경로로 기본 전환되고 음성 선택과 TUI 미디어 지원이 강화됐습니다.\n"
        "MCP 앱과 사용자 지정 서버 지원이 풍부해지고 실험적 codex exec-server 서브명령이 추가됐습니다.\n"
        "속도 제한 비동기 가져오기, /resume 안정성, macOS HTTP 클라이언트 패닉 등이 수정됐습니다.",
    ),
    (
        "4f92bbb5-f5ba-4129-860c-e1e9b5b480ec",
        "Realtime V2가 백그라운드 진행 상황을 스트리밍해요",
        "Realtime V2가 백그라운드 에이전트 진행 상황을 작업 중에 스트리밍하고 후속 응답을 큐에 넣습니다.\n"
        "TUI에서 실행 중인 후크가 완료된 출력과 분리되어 표시되고 코드 모드 도구 선언이 더 정확해집니다.\n"
        "Windows 샌드박스 분할 정책, 심볼릭 링크 카브아웃, codex --remote wss:// TLS 패닉이 수정됐습니다.",
    ),
    (
        "2aac4248-8ece-4851-8454-727dc773f27c",
        "플러그인 마켓플레이스와 프롬프트 히스토리가 추가됐어요",
        "GitHub, git URL, 로컬 디렉터리, marketplace.json URL에서 마켓플레이스를 설치할 수 있습니다.\n"
        "TUI에 Ctrl+R 역방향 검색과 슬래시 명령 호출 기록, 메모리 모드 제어가 추가됐습니다.\n"
        "MCP Apps 도구 호출, 네임스페이스 등록, 병렬 호출과 보안 devcontainer 프로필이 도입됐습니다.",
    ),
    (
        "796f8636-d501-4b56-a27d-328175d6d396",
        "Codex 앱에 인앱 브라우저와 컴퓨터 사용 기능이 들어왔어요",
        "Codex 앱이 로컬 페이지용 인앱 브라우저, macOS 컴퓨터 사용, 프로젝트와 무관한 Chats 스레드를 추가했습니다.\n"
        "스레드 자동화로 같은 스레드를 일정에 따라 깨울 수 있고 GitHub PR 검토 흐름이 앱에 들어왔습니다.\n"
        "아티팩트 뷰어로 PDF, 스프레드시트, 문서, 프레젠테이션을 미리 볼 수 있고 Intel Mac과 다중 창을 지원합니다.",
    ),
    (
        "d066e1b6-389e-480b-b8e7-f88849ac1dd8",
        "/side 대화와 플러그인 마켓플레이스 관리가 추가됐어요",
        "독립 설치가 더 자체 완결적이고 codex app이 Windows와 Intel Mac에서 데스크톱을 정확히 엽니다.\n"
        "TUI에 /side 대화와 슬래시 명령 큐가 추가되고 Plan Mode가 새 컨텍스트에서 구현을 시작할 수 있습니다.\n"
        "deny-read 글롭 정책, codex exec 격리 실행, 도구 발견과 이미지 생성 기본 활성화가 도입됐습니다.",
    ),
    (
        "350db16f-f597-4acc-bae9-b4835e9333af",
        "Amazon Bedrock 공급자와 /mcp verbose가 추가됐어요",
        "AWS 프로파일을 지원하는 amazon-bedrock 모델 공급자가 내장되고 /mcp verbose가 전체 진단을 제공합니다.\n"
        "플러그인 MCP 로딩이 mcpServers와 최상위 서버 맵을 모두 받고 실시간 핸드오프가 트랜스크립트 델타를 전달합니다.\n"
        "롤백 후 /copy, 수동 셸 중 후속 메시지 큐, WSL 터미널 유니코드 입력이 수정됐습니다.",
    ),
    (
        "0ebea43b-f35d-4031-80b3-05b272ce5e74",
        "추론 단계 단축키와 Bedrock 정식 지원이 추가됐어요",
        "TUI에 Alt+,/Alt+. 추론 단축키가 추가되고 모델 업그레이드 시 추론 단계가 새 모델 기본값으로 재설정됩니다.\n"
        "AWS SigV4 서명을 갖춘 Amazon Bedrock 정식 지원이 추가되고 후크가 안정 단계로 들어왔습니다.\n"
        "Cloudflare 쿠키 유지, 웹소켓 종료 시 데이터 누수, /permissions 사이드 대화 동기화가 수정됐습니다.",
    ),
    (
        "a54a0594-5df1-472d-9e5e-8d67698327db",
        "Codex에 GPT-5.5와 자동 검토 에이전트가 들어왔어요",
        "GPT-5.5가 Codex 권장 모델로 출시되어 코딩, 컴퓨터 사용, 지식 작업, 리서치 워크플로에 적용됩니다.\n"
        "Codex 앱에 로컬 개발 서버와 파일 기반 페이지를 위한 초기 인앱 브라우저가 추가됐습니다.\n"
        "승인 프롬프트가 자동 검토 에이전트를 통해 승인/거부/중지/타임아웃과 위험도를 표시할 수 있습니다.",
    ),
    (
        "623dd146-d91a-4709-8f0f-e89e2011bc49",
        "Unix 소켓 전송과 권한 프로파일 라운드트립이 추가됐어요",
        "앱 서버에 Unix 소켓 전송이 추가되고 원격 플러그인 마켓플레이스 설치/업그레이드를 지원합니다.\n"
        "권한 프로파일이 TUI/MCP/셸/앱 서버에서 라운드트립되고 모델 공급자가 자체 모델 발견을 처리합니다.\n"
        "검토/종료 멈춤, exec-server 출력 버퍼링, Windows 샌드박스 멀티 버전 PATH가 수정됐습니다.",
    ),
]
# fmt: on
