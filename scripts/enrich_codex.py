...
updates = [
    ...
    ("6d560a8b-79e5-4e3a-b344-48a5f039e93e",
     "Codex App 26.313: Navigation buttons & open-in-file-manager action",
     "Added back and forward buttons in the header for faster navigation between recent screens.\nNew Open in Finder/Explorer/File Manager action from thread menus to jump directly to a thread's project folder.\nImproved resume and thread error toasts with clearer details when something goes wrong."),
    ("a1021ca2-8647-4a3c-9409-192b2b23d0f1",
     "Codex App 26.317: Fork conversations, model slash commands & plan notifications",
     "Users can now fork a conversation from an earlier message, not just the latest turn, for branching explorations.\nAdded slash commands for switching models and reasoning levels that work mid-draft in the prompt composer.\nPlan mode questions now trigger notifications so it's easier to notice when Codex needs input."),
    ("9b679ced-3744-400d-989e-59c74252639f",
     "Codex App 26.318: Skills in @ menu & improved Cmd+F search",
     "Skills are now available in the @ menu for inserting them from the composer alongside other mentions.\nCmd/Ctrl+F search now starts with your current text selection for faster searching in reviews and diffs.\nIncludes additional performance improvements and bug fixes."),
    ("28c3194f-f50c-4b97-92a4-bd351eb7f70d",
     "Codex App 26.323: Thread search, bulk archive & settings sync",
     "Added search for past threads with a sidebar shortcut and keyboard shortcuts for jumping to recent conversations.\nOne-click option to archive all local threads in a project for workspace cleanup.\nKey settings now sync between the Codex app and VS Code extension with a new settings entry point."),
    ("0439ab54-453b-41fe-b97d-1f18b01b7c0c",
     "Codex Plugins: Installable bundles for skills, apps & MCP configs",
     "Codex now supports plugins as installable bundles packaging skills, app integrations, and MCP server configuration for reusable workflows.\nPlugins are available in the Codex app, CLI, and IDE extensions with a curated plugin directory for discovery.\nDevelopers can scaffold local plugins with @plugin-creator and test them with workspace-scoped or home-scoped marketplaces."),
    ("f6663849-8a9e-4a70-b660-974f82c4fa63",
     "Codex CLI 0.117.0: First-class plugins, path-based sub-agents & messaging",
     "Plugins are now a first-class workflow with startup sync, in-app browsing via /plugins, and clearer auth and setup handling.\nSub-agents use readable path-based addresses like /root/agent_a with structured inter-agent messaging.\nIncludes plugin lifecycle improvements for install, remove, and remote sync across workspaces."),
    # --- batch added 2026-04-26 ---
    ("cf8c51a4-2b01-4b4d-ac59-57ed7e3890f2",
     "Codex CLI 0.118.0: Windows proxy sandbox, device-code login & dynamic auth",
     "Windows sandbox runs can enforce proxy-only networking with OS-level egress rules, app-server clients can start ChatGPT sign-in with device codes, and codex exec accepts prompt-plus-stdin workflows.\nCustom model providers can fetch and refresh short-lived bearer tokens dynamically instead of relying on static credentials from config or environment variables.\nFixes initial .codex file creation safety, Linux bwrap discovery on multi-entry PATH, app-server-backed TUI hooks/copy/resume/agent regressions, and Windows apply_patch ACL churn."),
    ("ee46ada6-cec1-4271-b72f-903cfbed1943",
     "Codex Models: Remove older GPT-5.x models for ChatGPT sign-in users",
     "Starting April 7, the Codex model picker no longer shows gpt-5.2-codex, gpt-5.1-codex-mini, gpt-5.1-codex-max, gpt-5.1-codex, gpt-5.1, or gpt-5 for ChatGPT sign-in.\nOn April 14, those models will be fully removed from Codex for ChatGPT sign-in users; gpt-5.4, gpt-5.4-mini, gpt-5.3-codex, and gpt-5.2 remain available, with gpt-5.3-codex-spark for Pro.\nUsers can still access other API-supported models in Codex by signing in with an API key or configuring a custom model provider."),
    ("b009f808-ee86-46b4-9da2-fd56d6d0bd03",
     "Codex CLI 0.119.0: Realtime voice v2 WebRTC, MCP Apps & exec-server",
     "Realtime voice sessions default to v2 WebRTC with configurable transport, voice selection, native TUI media support, and full app-server coverage; MCP Apps and custom servers gain richer support.\nAdds egress websocket transport, remote --cd forwarding, runtime remote-control enablement, sandbox-aware filesystem APIs, and an experimental codex exec-server subcommand; Ctrl+O copies latest agent response.\nFixes async rate-limit fetching, /resume picker stability, fast mode toggle in app-server-backed TUI, and macOS HTTP-client sandbox panics; codex-core split into focused crates."),
    ("4f92bbb5-f5ba-4129-860c-e1e9b5b480ec",
     "Codex CLI 0.120.0: Streaming Realtime V2 & richer hook activity display",
     "Realtime V2 streams background agent progress while work runs and queues follow-up responses until the active response completes; custom TUI status lines now include the renamed thread title.\nHook activity in the TUI separates live running hooks from completed output, code-mode tool declarations include MCP outputSchema details, and SessionStart hooks distinguish /clear from fresh sessions.\nFixes Windows elevated sandbox split filesystem policies, symlinked writable root carveouts, codex --remote wss:// TLS panics, and tool search ordering preservation."),
    ("2aac4248-8ece-4851-8454-727dc773f27c",
     "Codex CLI 0.121.0: Plugin marketplaces, prompt history & MCP Apps tool calls",
     "Adds codex marketplace add for installing from GitHub, git URLs, local directories, and direct marketplace.json URLs, plus Ctrl+R reverse search and slash command recall in TUI.\nIntroduces TUI/app-server controls for memory mode, reset/deletion, and extension cleanup; expands MCP support with Apps tool calls, namespaced registration, and parallel-call opt-in.\nAdds a secure devcontainer profile with bubblewrap support and macOS Unix socket allowlists; fixes danger-full-access denylist removal, Windows verbatim path handling, and Guardian timeout signaling."),
    ("796f8636-d501-4b56-a27d-328175d6d396",
     "Codex App 26.415: In-app browser, computer use & GitHub PR workflow",
     "Adds an early in-app browser for local pages with comment support, computer use for macOS app testing (excluding EEA/UK/Switzerland), and Chats threads not tied to a project folder.\nThread automations wake the same thread on a schedule, the task sidebar surfaces plans/sources/artifacts, and GitHub PR review brings inspect/comment/explain/change workflows into the app.\nArtifact viewer previews PDFs/spreadsheets/documents/presentations before commit; remote SSH alpha, multi-terminal, menu bar/system tray, multi-window, Intel Mac support, and new plugins also ship."),
    ("d066e1b6-389e-480b-b8e7-f88849ac1dd8",
     "Codex CLI 0.122.0: /side conversations, plugin marketplaces & deny-read globs",
     "Standalone installs are more self-contained and codex app opens or installs Desktop on Windows/Intel Macs; TUI adds /side conversations and queued slash/!shell input.\nPlan Mode can start implementation in a fresh context with usage shown; plugin workflows gain tabbed browsing, inline toggles, marketplace removal, and remote/cross-repo/local sources.\nAdds deny-read glob policies, managed deny-read requirements, isolated codex exec runs, and default tool discovery and image generation; tightens logout token revocation and Windows sandbox SSH grants."),
    ("350db16f-f597-4acc-bae9-b4835e9333af",
     "Codex CLI 0.123.0: Amazon Bedrock provider, /mcp verbose & realtime handoff fixes",
     "Adds built-in amazon-bedrock model provider with AWS profile support, /mcp verbose for full MCP diagnostics, and host-specific remote_sandbox_config requirements.\nPlugin MCP loading accepts both mcpServers and top-level server maps; realtime handoffs deliver transcript deltas to background agents with explicit silence support.\nFixes /copy after rollback, queued follow-ups during manual shells, Unicode/dead-key input in WSL terminals, and codex exec inheriting root-level shared flags."),
    ("0ebea43b-f35d-4031-80b3-05b272ce5e74",
     "Codex CLI 0.124.0: Quick reasoning controls, Bedrock support & stable hooks",
     "TUI gets Alt+,/Alt+. reasoning shortcuts and resets reasoning to model defaults on accepted upgrades; app-server sessions can manage multiple environments per turn.\nAdds first-class Amazon Bedrock support with SigV4 signing, remote plugin marketplace listing, and stable hooks configurable in config.toml that observe MCP, apply_patch, and Bash sessions.\nFixes Cloudflare cookie persistence, websocket drainage on shutdown, /permissions drift across side conversations, wait_agent prompt response, and managed config startup edge cases."),
    ("a54a0594-5df1-472d-9e5e-8d67698327db",
     "GPT-5.5 in Codex: New default model, in-app browser & automatic reviewer agent",
     "GPT-5.5 launches as Codex's recommended model for coding, computer use, knowledge work, and research workflows across CLI, IDE extension, and Codex app.\nThe Codex app gains an early in-app browser for local dev servers and file-backed pages, controlled via the bundled Browser plugin with allowed/blocked website management.\nApproval prompts can route through an automatic reviewer agent that returns approve/deny/stop/timeout decisions with risk levels before the request runs."),
    ("623dd146-d91a-4709-8f0f-e89e2011bc49",
     "Codex CLI 0.125.0: Unix socket transport, permission profiles & rollout tracing",
     "Adds Unix socket transport for app-server, remote plugin marketplace install/upgrade, and permission profile round-tripping across TUI/MCP/shell/app-server.\nModel providers now own discovery with AWS Bedrock account state exposed; codex exec --json reports reasoning-token usage; rollout tracing records tool/code-mode/session relationships.\nFixes review/exit wedging, exec-server output buffering, untrusted project trust persistence, and Windows sandbox startup with multi-version PATH handling."),
]
